  ...
  <div v-if="post" :class="`post-preview-card ${getCardStyle}`">
    ...
    <div :class="getPostInfoContainerStyle">
      ...
      <div :class="showMinimalContent ? 'hidden md:block' : ''">
        <nuxt-content ref="excerpt" :document="excerpt" :editable="false" :class="`prose leading-snug prose-a:text-inherit prose-a:no-underline dark:prose-invert transition prose-code:before:content-none prose-code:after:content-none ${showMinimalContent ? 'minimal-preview-text' : ''}`" />

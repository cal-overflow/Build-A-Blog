  ...
  <div v-if="post" :class="`post-preview-card ${getCardStyle}`">
    ...
    <div :class="getPostInfoContainerStyle">
      ...
      <div :class="showMinimalContent ? 'hidden md:block' : ''">
        ...
        <nuxt-link ref="title" :to="`${currentRoute}/${post.slug}`" :class="`text-extra-gray-dark dark:text-extra-gray-light font-thin text-sm underline hover:no-underline transition ${showMinimalContent ? 'hidden md:block': ''}`">
          Continue reading
        </nuxt-link>
      </div>
    </div>
